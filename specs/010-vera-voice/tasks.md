...
- [ ] T023 [US1] [MARKETING] `verbs.py` book / reschedule / availability into the existing pipeline (`booking_agent.confirm_booking`/`cancel_booking`, `availability_agent`) — read-back before write, **idempotent**, no bypass writes (FR-008/FR-009/FR-010). (deps: T015)
...
- [ ] T024 [US1] Unverified-scope enforcement in `verbs.py` — unverified callers limited to `availability` + `intake_capture`; never read/act on an existing client's records (FR-006). (deps: T023, T006)
